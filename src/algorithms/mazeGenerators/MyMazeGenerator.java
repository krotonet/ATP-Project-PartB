...
public class MyMazeGenerator extends AMazeGenerator{
    ...
    public List<Position> getNeighbors(Maze maze, Position position){
        List<Position> cells = new LinkedList<>();
        if(position.getColumnIndex() - 2 >= 0 && maze.getValue(position.getRowIndex(),position.getColumnIndex() - 2) == 1) {
            cells.add(new Position(position.getRowIndex(),position.getColumnIndex() - 2));
        }
        if(position.getColumnIndex() + 2 < maze.getColumns() && maze.getValue(position.getRowIndex(),position.getColumnIndex() + 2) == 1) {
            cells.add(new Position(position.getRowIndex(),position.getColumnIndex() + 2));
        }
        if(position.getRowIndex() - 2 >= 0 && maze.getValue(position.getRowIndex() - 2, position.getColumnIndex()) == 1) {
            cells.add(new Position(position.getRowIndex() - 2,position.getColumnIndex()));
        }
        if(position.getRowIndex() + 2 < maze.getRows() && maze.getValue(position.getRowIndex() + 2, position.getColumnIndex()) == 1) {
            cells.add(new Position(position.getRowIndex() + 2,position.getColumnIndex()));
        }
